...
function displayQuote(responseJson) {
  ...
  let finalString =
    ...
    <h3 class="` +
    determinePriceColor(priceChange) +
    `">Current Price:  $` +
    price +
    `</h3>
    <h5 class="` +
    determinePrevPriceColor(previousPrice, price) +
    `">Previous Price:  $` +
    previousPrice +
    ` on ` +
    lastTradingDay +
...
function getQuote(symbol) {
  ...
  fetch(url)
    ...
    .catch((error) =>
      alert(
        `Error Message2: ${error.message} - REQUEST LIMIT REACHED - PLEASE ALLOW 60 SECONDS BEFORE NEXT SEARCH`
      )
    );
}
...
function displayNews(responseJson) {
  ...
  for (let i = 0; i < responseJson.value.length || i === 24; i++) {
    ...
    let newsImage = responseJson.value[i].image.url;
    console.log(newsImage, typeof newsImage);
    let tempNewsImage = "news.png";
    ...
    $("#newsList").append(
      `<li class="newsItem"><div class="newsItem-img"><img src='${newsImage}' alt="news image" onerror="this.onerror=null;this.src='${tempNewsImage}';" ></div><a href='${url}' target="_blank">
      <h3>${title}</h3></a></li>`
    );
  }
...
function getCompany(symbol) {
  ...
  fetch(url)
    ...
    .catch((error) =>
      alert(
        `Error Message3: ${error.message} - REQUEST LIMIT REACHED - PLEASE ALLOW 60 SECONDS BEFORE NEXT SEARCH`
      )
    );
}
...
function searchCompanies(company) {
  ...
  fetch(url)
    ...
    .catch((error) =>
      alert(
        `Error Message1: ${error.message} - REQUEST LIMIT REACHED - PLEASE ALLOW 60 SECONDS BEFORE NEXT SEARCH`
      )
    );
}
